...
@MainActor
struct CategoryNotesViewModelTests {

    @Test func fetchNotes_success_updatesStateToLoadedWithOnlyMatchingCategory() async {
        let category = Category("Compras", color: "RED")
        let otherCategory = Category("Viajes", color: "BLUE")
        let note = Note("Super", value: "Leche", category: category, createdAt: .now)
        let otherNote = Note("Vuelo", value: "Reservar", category: otherCategory, createdAt: .now)
        let sut = CategoryNotesViewModel(category: category, useCase: MockNoteUseCase(notes: [note, otherNote]))
    ...
    @Test func fetchNotes_failure_updatesStateToError() async {
        let category = Category("Compras", color: "RED")
        let sut = CategoryNotesViewModel(category: category, useCase: ThrowingNoteUseCase())
    ...
    @Test func delete_removesNoteAndRefreshesList() async {
        ...
        let sut = CategoryNotesViewModel(category: category, useCase: useCase)
        await sut.fetchNotes()
